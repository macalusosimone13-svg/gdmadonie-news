...
export function useGdStats() {
  return useQuery({
    queryKey: ['gd-stats'],
    queryFn: async () => {
      // Comunicati e News GD sono due cose diverse per Simone (stesso
      // criterio gia' usato in GDMadonie.jsx: due tab separati), quindi qui
      // si contano separatamente invece di sommarli in un unico numero.
      const [comunicati, news, eventi] = await Promise.all([
      sb44.entities.Post.filter({ status: 'published', source_type: 'gd_madonie', category: 'comunicato' }, '-published_date', 200),
      sb44.entities.Post.filter({ status: 'published', source_type: 'gd_madonie', category: 'news_gd' }, '-published_date', 200),
      sb44.entities.Event.list('-date', 200)]);
      return { comunicati: comunicati.length, news: news.length, eventi: eventi.length };
    },
...
export function FollowStats({ withStats = true }) {
  ...
  const hasStats = withStats && stats && (stats.comunicati > 0 || stats.news > 0 || stats.eventi > 0);
  ...
  return (
    <>
      ...
      {hasStats &&
      <div className="side-box"><div className="counters"><div><b>{stats.comunicati}</b><span>Comunicati</span></div><div><b>{stats.news}</b><span>News</span></div><div><b>{stats.eventi}</b><span>Eventi</span></div></div></div>}
    </>);
}
